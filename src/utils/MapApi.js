...
export const GOOGLE_MAP_API = async (payload) => {
    ...
    const url = 'https://maps.googleapis.com/maps/api/geocode/json?address=' + latitude + ',' + longitude + '&key=AIzaSyAQDBt0FHmbPe6x7O6H5EudhcdaaFXO_H4'
    ...
    try {
        return await axios.get(url, { cancelToken: cancelToken.token }).then(function (response) {
            const { data } = response
